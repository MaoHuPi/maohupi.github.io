...
(module
	...
	(import "Math" "pow" (func $Math.pow (param f32) (param f32) (result f32)))
	...
	(func $pow
		...
		(local.set $y (call $Math.pow (f32.abs (local.get $x)) (local.get $y)))
		(if (f32.lt (local.get $x) (f32.const 0))
			(then
				(local.set $y (f32.neg (local.get $y)))
			)
		)
		(local.get $y)
	)
				...
				(call $pow
					(f32.sub
						(f32.mul
							...
							(f32.const 2))
						(f32.const 1))
					(f32.div (f32.const 1) (f32.const 3)))
				(f32.const 1))
			(f32.const 2))
	)
	(func $stretchImage
			...
			(local.set $11Ptr
				(i32.add
					...
					(i32.mul
						...
						(local.get $channelAmount))))
			(local.set $ci (i32.const 0))
			;; (local.set $wr (call $stretchSmoothCurl (f32.sub (local.get $wr) (f32.floor (local.get $wr)))))
			;; (local.set $hr (call $stretchSmoothCurl (f32.sub (local.get $hr) (f32.floor (local.get $hr)))))
