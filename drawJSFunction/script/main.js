...
%end`;
if('exam' in $_GET){
    sendXmlhttp(`example/${$_GET['exam']}.js`, '', t => {
        inputFunc.value = t;
    }, 'get');
}
else if('code' in $_GET){
    inputFunc.value = $_GET['code'];
}
...
function draw(){
    ...
    let settingsData = {
        ...
        name: {regexp: /%(line)*[Nn]ame=(.[^ ]*)/g, default: '', method: ((v, func) => {
            try{
                let p = [0, viewRect[1] + cvs.height - func(viewRect[0])]
                ctx.save();
                desSize = 35;
        ...
        waveFun: {regexp: /%wave([Ff]un)*=(.[^ ]*)/g, default: false, method: ((v, func) => {
            if(typeof(v) == 'string'){
                v = JSON.parse(v);
            }
            if(typeof(v) == 'number'){
    ...
    if(drawReferenceGrid){
        ...
        for(let i = viewRect[0]; i < viewRect[2]; i++){
            if(i%100 == 0){
                ...
                ctx.fillText(i, x-5, cvs.height-5);
            }
        }
        for(let i = viewRect[1]; i < viewRect[3]; i++){
            if(i%100 == 0){
                ...
                ctx.fillText(i, cvs.width-5, y-5);
            }
        }
        ctx.stroke();
        ctx.restore();
            ...
            for(let settingsType in settingsData){
                ...
                if(vList != null){
                    ...
                    for(let v of vList){
                        ...
                        try{
                            (settingsData[settingsType]['method'])(JSON.parse(v), func);
                        }
                        catch(e){
                            (settingsData[settingsType]['method'])(eval(`\`${v}\``, time = time, deg = rad => rad/360*(Math.PI*2)), func);
                        }
                    }
                }
            }
...
function waveLoop(){
    try{
        if(waveFun){
            wavePlayer.changeFrequency(waveFun(waveX));
            wavePlayer.volumeController.gain.value = wavePlayer.maxVolume;
        }
        else{
            wavePlayer.volumeController.gain.value = 0;
        }
    }
    catch(e){}
    setTimeout(waveLoop, 1);
}
waveLoop();
function playSound(){
    wavePlayer.start();
