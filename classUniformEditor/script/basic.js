/*
 * 2022 © MaoHuPi
 * classUniformEditor/script/basic.js
 */
...
const basicYear = new Date(0).getFullYear();
function getDeltaTimeText(date){
    let deltaTime = new Date().getTime() - new Date(date).getTime();
    let carryData = [[1e3, '秒'], [60, '分鐘'], [60, '小時'], [24, '天'], [30, '個月'], [365, '年']];
    let deltaTimeText = '剛剛';
    let i = 0;
    while(deltaTime / carryData[i][0] >= 1){
        deltaTime /= carryData[i][0];
        deltaTimeText = `${Math.floor(deltaTime).toString()}${carryData[i][1]}前`;
        i++;
    }
    return(deltaTimeText);
}
...
function openFile(type = '*', onchange = () => {}){
